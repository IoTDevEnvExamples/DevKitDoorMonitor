...
namespace IoTWorkbench
{
    public static class door_monitor
    {
        ...
        public static void Run([IoTHubTrigger("%eventHubConnectionPath%", Connection = "eventHubConnectionString")]EventData message, ILogger log)
        {
            ...
            var response = client.PostAsync("https://prod-07..yourLogicAppURL..", httpContent);

        if (!response.IsCompletedSuccessfully)
        {
            // Show the error for the failure.
            var errorMessage = response.AsyncState.ToString();
            throw new Exception(
                $"SendGrid failed to send email. Code: {response.Status}, error: {errorMessage}");
        }            
        }
    }
}